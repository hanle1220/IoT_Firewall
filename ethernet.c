    ...
    if (kbhitUart0())
    {
        ...
        else
        {
            ...
            if (strcmp(token, "help") == 0)
            {
                ...
                putsUart0("  allow mac w.x.y.z udp|tcp|mqtt|dhcp\n");
                putsUart0("  deny mac w.x.y.z udp|tcp|mqtt|dhcp\n");
            }
        }
    }
}
...
int main(void)
{
    ...
    etherSetMacAddress(2, 3, 4, 5, 6, 105);

    // Init ethernet interface (eth1)
    putsUart0("\nStarting eth1\n");
    ether1Init(ETHER_UNICAST | ETHER_BROADCAST | ETHER_HALFDUPLEX);
    ...
    while (true)
    {
        ...
        if (etherIsDataAvailable())
        {
            ...
            ether1GetPacket(data, MAX_PACKET_SIZE);
            //etherGetIpAddress(ip);
            //ether1SetIpAddress(ip);
            //ether1PutPacket(data, MAX_PACKET_SIZE);
            //etherPutPacket(data, MAX_PACKET_SIZE);

            // Handle ARP request
            if (etherIsArpRequest(data))
                ether1SendArpResponse(data);
            	...
            	if (etherIsIpUnicast(data))
            	{
                    ...
                    if (etherIsUdp(data))
                    {
                        ...
                        if (strcmp((char*)udpData, "off") == 0)
                            setPinValue(GREEN_LED, 0);
                        ether1SendUdpResponse(data, (uint8_t*)"Received", 9);
                    }

                    //Process TCP packet
                    if(etherIsTcp(data))
                    {
                        if(tcpIsPortOpen(data))
                        {
                            if(tcpGetState() == TCP_LISTEN && tcpIsSyn(data))
                            {
                                 ...
                                 s.remotePort = tcp->sourcePort;
                                 s.sequenceNumber = 0;
                                 tcpSendResponse(data, &s, TCPACK | TCPSYN);
                                 s.sequenceNumber++;
                                 tcpSetState(TCP_SYN_RECEIVED);
                            }
                            else if(tcpGetState() == TCP_SYN_RECEIVED && tcpIsAck(data))
                            {
                                tcpSetState(TCP_ESTABLISHED);
                                startPeriodicTimer(flash, 10);
                            }
                            else if(tcpGetState() == TCP_ESTABLISHED)
                            {
                                tcpSendResponse(data, &s, TCPACK);
                            }
                            else
                                tcpSendResponse(data, &s, TCPACK|TCPRST);
