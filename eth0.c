...
void etherCalcIpChecksum(ipHeader* ip)
{
    ...
    uint32_t sum = 0;
    ip->headerChecksum = 0;
    etherSumWords(ip, ((ip->revSize & 0xF) * 4), &sum);
...
// Sets IP address
void etherSetIpAddress(const uint8_t ip[4])
{
    uint8_t i;
    for (i = 0; i < 4; i++)
        ipAddress[i] = ip[i];
...
// Sets IP subnet mask
void etherSetIpSubnetMask(const uint8_t mask[4])
{
    uint8_t i;
    for (i = 0; i < 4; i++)
...
void etherSetIpGatewayAddress(const uint8_t ip[4])
{
    uint8_t i;
    for (i = 0; i < 4; i++)
        ipGwAddress[i] = ip[i];
...
// Sets IP DNS address
void etherSetIpDnsAddress(const uint8_t ip[4])
{
    uint8_t i;
    for (i = 0; i < 4; i++)
        ipDnsAddress[i] = ip[i];
...
// Gets IP gateway address
void etherGetIpDnsAddress(uint8_t ip[4])
{
    uint8_t i;
    for (i = 0; i < 4; i++)
        ip[i] = ipDnsAddress[i];
...
void etherSetIpTimeServerAddress(const uint8_t ip[4])
{
    uint8_t i;
    for (i = 0; i < 4; i++)
...
void etherGetIpTimeServerAddress(uint8_t ip[4])
{
    uint8_t i;
    for (i = 0; i < 4; i++)
